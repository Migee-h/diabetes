<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="设备名称或型号" prop="deviceName">
        <el-input
      ...
      <el-form-item label="设备类型" prop="deviceType">
        <el-select v-model="queryParams.deviceType" placeholder="请选择设备类型" clearable>
          <el-option
            v-for="dict in dict.type.device_category"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="infoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="设备名称或型号" align="center" prop="deviceName" />
      <el-table-column label="设备序列号" align="center" prop="serialNumber" />
      <el-table-column label="设备类型" align="center" prop="deviceType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.device_category" :value="scope.row.deviceType"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="设备名称或型号" prop="deviceName">
          <el-input v-model="form.deviceName" placeholder="请输入设备名称或型号" />
        </el-form-item>
        <el-form-item label="设备序列号" prop="serialNumber">
...
<script>
import { listInfo, getInfo, delInfo, addInfo, updateInfo } from "@/api/archive/device/info";

export default {
  name: "Info",
  dicts: ['device_category'],
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        deviceName: [
          { required: true, message: "设备名称或型号不能为空", trigger: "blur" }
        ],
        serialNumber: [
